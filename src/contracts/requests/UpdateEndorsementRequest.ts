...
export const zUpdateEndorsementRequest = z.object({
    ...
    handle: z.string().max(50).nullish(),
    quote: z.string().nonempty().max(300).nullish(),
    websiteHref: z.string().nullish(),
    donateHref: z.string().nullish(),
    imgHref: z.string().max(200).nullish(),
    isPvMember: z.boolean().optional(),
    primaryElectionDate: z.coerce.date().nullish(),
    generalElectionDate: z.coerce.date().nullish(),
