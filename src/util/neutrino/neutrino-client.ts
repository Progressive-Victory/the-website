...
export class NeutrinoClient {
    ...
    async verifySecurityCode(
        ...
        const response = await this.fetch(
            'POST',
            NeutrinoRoutes.VerifySecurityCode,
            formData
        )
    ...
    async smsVerify(
        ...
        if (options) {
            if (
                ...
                formData.set(
                    'code-length',
                    Math.round(options.codeLength).toString()
                )
            }
            if (options.securityCode)
                formData.set(
                    'security-code',
                    Math.round(options.securityCode).toString()
                )
            if (options.brandName) formData.set('brand-name', options.brandName)
            if (options.countryCode)
                formData.set('country-code', options.countryCode)
            if (options.languageCode)
                formData.set('language-code', options.languageCode)
            if (options.limit)
                formData.set('limit', Math.round(options.limit).toString())
            if (options.limitTTL)
                formData.set(
                    'limit-ttl',
                    Math.round(options.limitTTL).toString()
                )
        }

        const response = await this.fetch(
            'POST',
            NeutrinoRoutes.SmsVerify,
            formData
        )
