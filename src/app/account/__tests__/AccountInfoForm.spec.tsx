...
describe('AccountInfoForm', () => {
	...
	it.each(Object.entries(immutableFields))("immutable fields are disabled: %s", (name, value) => {
	...
	it.each(accountInfoFieldNames)("renders form label correctly: %s", (label) => {
	...
	it.each(Object.entries(testUserData))("renders initial %s value correctly", (name, value) => {
		...
		if (name === "dateOfBirth") {
			expect(getByDisplayValue(value.toISOString().split('T')[0])).toBeVisible()
		} else if (name === "state") {
	...
	it.each(Object.entries(mutableFields))('Save Changes button until a field is changed: %s', async (name, value) => {
