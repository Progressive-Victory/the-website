...
export function List(props: ListProps) {
    ...
    const [prominentScrolled, setProminentScrolled] = useState(false)
    const [isInProminentSidebar, setIsInProminentSidebar] = useState(false)
    ...
    useEffect(() => {
        ...
        const prominentSidebar = listElement.closest<HTMLElement>(
            '[data-sidebar-variant="prominent"]'
        )
        setIsInProminentSidebar(!!prominentSidebar)
    }, [])
    ...
    useEffect(() => {
        ...
        prominentSidebar.dataset.prominentScrolled = prominentScrolled
            ? 'true'
            : 'false'

        return () => {
            prominentSidebar.dataset.prominentScrolled = 'false'
        }
    }, [isInProminentSidebar, prominentScrolled])

    const handleElementScrollStateChange = (scrolled: boolean) => {
        if (!isInProminentSidebar) {
            if (prominentScrolled) {
                setProminentScrolled(false)
            }
            return
        }

        setProminentScrolled(scrolled)
    }

    return (
        <div
            className={styles.list}
            data-prominent-scrolled={
                isInProminentSidebar && prominentScrolled ? 'true' : 'false'
            }
            ref={listRef}
        >
            <ListTop
            ...
            <ListBody
                {...props}
                onElementScrollStateChange={handleElementScrollStateChange}
            />
...
> & {
    onElementScrollStateChange?: (scrolled: boolean) => void
}
...
export function ListBody({
    ...
    pinnedContent,
    children,
    onElementScrollStateChange,
}: ListBodyProps) {
    useEffect(() => {
        if (count == null) {
            onElementScrollStateChange?.(false)
        }
    }, [count, onElementScrollStateChange])

    const handleElementListScroll = (event: UIEvent<HTMLUListElement>) => {
        onElementScrollStateChange?.(event.currentTarget.scrollTop > 8)
    }
    ...
    return (
        <>
            ...
            <ul
                ...
                onScroll={handleElementListScroll}
            >
                {children}
            </ul>
        </>
