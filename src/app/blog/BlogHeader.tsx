...
export default function BlogHeader() {
      ...
      <div>
        {words.map((word, index) => (
          <div key={index} className="inline-block relative">
            <span
              className="tracking-wide absolute bottom-1 right-1"
  ...
  return (
    <div className="bg-white grid grid-cols-2 py-5 text-4xl">
      <h1 className="grid sm:text-5xl md:text-6xl lg:text-7xl text-right content-evenly font-extrabold text-[#09223a]">
        {DoubleTextEffect("Progressive")}
        {DoubleTextEffect("Victory")}
        {DoubleTextEffect("Blog", "#4483C7", "#09223a")}
      </h1>
      <div className="inline-block relative">
        <Logo className="w-60 absolute bottom-1 left-1" />
        <Logo className="w-60" color="#4483C7" />
      </div>
    </div>
  );
}
